...
def minimax(side, board, flags, depth):
    ...
    for move in moves:
        ...
        mvalue, mlist, mtree = minimax(newside, newboard, newflags, depth - 1)
        
        if(side):
          if(mvalue < return_val):
            return_val = mvalue
            mlist.insert(0, move)
            return_list = mlist
        else:
          if(mvalue > return_val):
            ...
            mlist.insert(0, move)
            return_list = mlist
        return_tree[encode(*move)] = mtree
        
    return (return_val, return_list, return_tree)
...
def alphabeta(side, board, flags, depth, alpha=-math.inf, beta=math.inf):
    ...
    for move in moves:
        ...
        mvalue, mlist, mtree = alphabeta(newside, newboard, newflags, depth - 1, alpha, beta)
